...
// SkillHandler serves the AI agent skill guide for LLMs helping users with Cinch
func SkillHandler(w http.ResponseWriter, r *http.Request) {
	// Log SKILL.md downloads for analytics
	ip := ExtractClientIP(r)
	ua := r.Header.Get("User-Agent")
	slog.Info("skill guide download", "ip", ip, "ua", ua)

	w.Header().Set("Content-Type", "text/markdown; charset=utf-8")
	w.Header().Set("Cache-Control", "no-cache")
	_, _ = w.Write([]byte(skillGuide))
}

const skillGuide = `# AI Agent Guide for Cinch
